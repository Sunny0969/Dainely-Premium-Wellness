...
class SyncProductJob implements ShouldQueue
{
    ...
    protected function upsertProduct(): void
    {
        ...
        // Keep admin "unpublished" — do not resurrect from Shopify webhook.
        if ($product && $product->status === \App\Support\ProductVisibility::STATUS_UNPUBLISHED) {
        ...
        // Re-warm handle + catalog from webhook JSON so storefront skips live Shopify API.
        app(LocalShopifyCatalog::class)->rememberWebhookProduct($this->payload);
    ...
    protected function deleteProduct(): void
    {
        ...
        if ($product) {
            ...
            $this->invalidateCaches($product);
            $product->delete();
            app(LocalShopifyCatalog::class)->forgetWebhookProduct($handle);
            Log::info('Product deleted from Supabase', [
                'shopify_product_id' => $shopifyProductId,
            ]);
        } elseif (! empty($this->payload['handle'])) {
            app(LocalShopifyCatalog::class)->forgetWebhookProduct((string) $this->payload['handle']);
        }
    }

    protected function invalidateCaches(Product $product): void
    {
        \App\Support\StorefrontCache::forgetProduct((string) $product->handle, (int) $product->id);
