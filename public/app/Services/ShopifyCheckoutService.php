...
class ShopifyCheckoutService
{
    ...
    public function createCheckout(array $items, ?array $address = null, ?string $email = null, ?string $discountCode = null): array
    {
        ...
        foreach ($items as $item) {
            ...
            if (! $variantId) {
                $resolved = $shopify->resolveCartVariantId(is_array($item) ? $item : []);
                $variantId = $resolved !== null ? (string) $resolved : null;
            }

            if (! $variantId) {
                Log::warning('Shopify checkout: cart line missing variant_id', [
...
GRAPHQL;
    ...
    protected function graphql(string $query, array $variables = []): array
    {
        try {
            ...
            Log::error('Shopify Storefront API exception', ['error' => $e->getMessage()]);
