@extends('layouts.app')
@section('title', 'Understanding Nerve Discomfort: Causes, Symptoms & Relief | Dainely')
@section('meta_description', 'Complete guide to Nerve Discomfort: why leg pain comes from your spine, the 3 root causes, and clinically proven decompression relief methods.')
@section('content')
{{-- Hero --}}
<section class="bg-navy-900 text-white py-16">
  <div class="container-site">
    <nav class="flex items-center gap-2 text-sm text-navy-300 mb-8">
      ...
      <span>/</span><span class="text-white">Nerve Discomfort</span>
    </nav>
    <div class="grid lg:grid-cols-2 gap-12 items-center">
      <div>
        <p class="eyebrow-light mb-4">Education Centre</p>
        <h1 class="font-display font-bold text-4xl lg:text-5xl text-white mb-6 leading-tight">The Science of Nerve Discomfort: Why Your <em class="text-gold-400">Leg</em> Hurts When Your Back Is the Problem</h1>
        <p class="text-navy-200 text-lg leading-relaxed mb-8">Nerve Discomfort affects 40% of people at some point in their lives, yet it remains widely misunderstood. Many patients treat the symptom — leg pain — without addressing the spinal compression causing it.</p>
      ...
      <div class="relative">
        <div class="bg-navy-800 rounded-3xl overflow-hidden">
          <img src="{{ asset('images/Nerve Discomfort-edu.png') }}" alt="Sciatic nerve anatomy" class="w-full h-80 object-cover opacity-90">
        </div>
      </div>
    </div>
  </div>
...
<section class="bg-white py-10 border-b border-slate-100">
  <div class="container-site grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
    @foreach([['40%','Adults affected by Nerve Discomfort'],['90%','Cases resolve with proper treatment'],['L4-S1','Most affected spinal levels'],['2-4 wk','Average recovery with decompression']] as [$v,$l])
...
<section class="section bg-white">
  <div class="container-site grid lg:grid-cols-3 gap-12">
    <div class="lg:col-span-2 prose prose-slate max-w-none">
      <h2>What Is Nerve Discomfort?</h2>
      <p>Nerve Discomfort is not a diagnosis — it's a symptom. The term describes pain, tingling, or numbness that travels along the path of the sciatic nerve, which runs from your lower back through your hips, buttocks, and down each leg.</p>
      ...
      <h2>The 3 Root Causes of Nerve Discomfort</h2>
    ...
    <div class="space-y-6">
      @include('partials.shopify-product-sidebar', [
        'product' => $product,
        'heading' => 'Recommended for Nerve Discomfort Relief',
