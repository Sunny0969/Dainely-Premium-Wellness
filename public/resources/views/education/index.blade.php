...
<section class="py-16 lg:py-24 bg-slate-50">
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-12">
            <span class="px-4 py-2 rounded-full bg-navy-600 text-white text-sm font-semibold shadow-sm cursor-pointer">
                All Articles
            </span>
            @php
                $allCategories = [
            ...
            @foreach($allCategories as $cat)
                <span class="px-4 py-2 rounded-full bg-white border border-slate-200 text-slate-600 text-sm font-semibold shadow-sm hover:border-navy-200 hover:text-navy-700 transition-colors cursor-pointer">
                    {{ $cat }}
                </span>
            @endforeach
        </div>
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($pages as $page)
                        <a href="{{ route('education.show', ['locale' => $locale, 'slug' => $page->slug]) }}" class="bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-navy-200 transition-all group flex flex-col h-full overflow-hidden">
                            @if($page->hero_image)
                                <img src="{{ asset('images/' . $page->hero_image) }}" alt="{{ $page->title }}" class="w-full h-48 object-cover">
                            @else
