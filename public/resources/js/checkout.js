...
export default function registerCheckoutForm(Alpine) {
  Alpine.data('checkoutForm', () => {
    ...
    return {
      ...
      cardReady: false,
      checkoutReady: false,
      cartLoading: Boolean(cfg.asyncCart),
      paymentError: '',
      paymentSuccess: '',
      ...
      freeShipRemainingTpl: cfg.freeShipRemainingTpl || 'Add :amount more for free shipping',
      discountCode: String(cfg.pendingDiscount || '').trim(),
      discountMessage: '',
      discountValid: false,
      discount: 0,
      ...
      calcShipping() {
        const threshold = Number(this.pricing.free_shipping_threshold ?? 29.99);
      ...
      freeShippingHint() {
        ...
        if (sub >= threshold) {
          return this.freeShipQualifies;
        }
        const remaining = Math.max(0, threshold - sub);
        const tpl = this.freeShipRemainingTpl || 'Add :amount more for free shipping';
        return tpl.replace(':amount', this.formatMoney(remaining));
      },
      ...
      applyClientConfig() {
        ...
        this.freeShipRemainingTpl = fresh.freeShipRemainingTpl || this.freeShipRemainingTpl;
      ...
      async syncCartSession(extra = {}) {
        ...
        try {
          ...
          if (Array.isArray(data.cartItems)) {
            this.applyCartSummaryPayload(data);
          }
        } catch (error) {
      ...
      bootstrapCheckoutState() {
        ...
        const sourceItems = fresh.cartItems ?? [];
      ...
      readCachedCartSummary() {
        try {
          const raw = localStorage.getItem('dainely_cart_summary');
          if (!raw) return null;
          const parsed = JSON.parse(raw);
          if (!parsed || !Array.isArray(parsed.cartItems) || !parsed.cartItems.length) return null;
          // Ignore stale cache (> 2 hours)
      ...
      writeCachedCartSummary(payload) {
        try {
          localStorage.setItem('dainely_cart_summary', JSON.stringify({
            cartItems: payload.cartItems || [],
            pricing: payload.pricing || {},
            summarySubtotal: payload.summarySubtotal,
            summarySubtotalUsd: payload.summarySubtotalUsd,
            savedAt: Date.now(),
          }));
        } catch (e) {
      ...
      applyCartSummaryPayload(data) {
        if (!data || data.success === false) return;
        if (Array.isArray(data.cartItems)) {
          this.cartItems = data.cartItems.map((item) => this.normalizeCartItem(item));
        }
        if (data.pricing && typeof data.pricing === 'object') {
          this.pricing = { ...(this.pricing || {}), ...data.pricing };
        }
        if (Number(data.summarySubtotal) > 0) {
          this.pricing.subtotal = Number(data.summarySubtotal);
        }
        if (Number(data.summarySubtotalUsd) > 0) {
          this.pricing.subtotal_usd = Number(data.summarySubtotalUsd);
        }
        this.shippingCost = Number(data.shippingCost ?? this.pricing?.shipping ?? this.calcShipping());
        this.taxAmount = Number(data.taxAmount ?? this.pricing?.tax ?? 0);
        if (this.taxAmount <= 0 && this.subtotal() > 0) {
          this.applyFallbackTax();
        }
        this.lockDisplayCurrency();
        this.writeCachedCartSummary(data);
        window.__CHECKOUT__ = {
          ...(window.__CHECKOUT__ || {}),
          cartItems: this.cartItems,
          pricing: this.pricing,
          summarySubtotal: this.subtotal(),
          summarySubtotalUsd: this.subtotalUsd(),
          shippingCost: this.shippingCost,
          taxAmount: this.taxAmount,
        };
      },
      async fetchCartSummary() {
        const url = checkoutCfg().urls?.cartSummary;
        if (!url) {
        ...
        try {
          ...
          if (data.empty && data.redirect) {
            window.location.href = data.redirect;
            return;
          }
          this.applyCartSummaryPayload(data);
        } catch (error) {
          console.error('[Dainely] cart summary failed', error);
        } finally {
          this.cartLoading = false;
          this.checkoutReady = true;
        }
      ...
      async init() {
        ...
        const wantsAsync = Boolean(checkoutCfg().asyncCart)
          || !(Array.isArray(checkoutCfg().cartItems) && checkoutCfg().cartItems.length);

        if (wantsAsync) {
          const cached = this.readCachedCartSummary();
          if (cached) {
            this.applyCartSummaryPayload({
              success: true,
              cartItems: cached.cartItems,
              pricing: cached.pricing,
              summarySubtotal: cached.summarySubtotal,
              summarySubtotalUsd: cached.summarySubtotalUsd,
            });
          }
      ...
      async ensureSquareSdk() {
        if (window.Square) {
          return true;
        }
        const src = checkoutCfg().squareSdkUrl || 'https://web.squarecdn.com/v1/square.js';
        if (document.querySelector(`script[data-dainely-square="1"]`)) {
          // Script already injecting — wait briefly for window.Square
          for (let i = 0; i < 50; i++) {
        ...
        await new Promise((resolve, reject) => {
          ...
          script.async = true;
          script.defer = true;
          script.dataset.dainelySquare = '1';
      ...
      async initSquare() {
        try {
          ...
          try {
            const loaded = await this.ensureSquareSdk();
