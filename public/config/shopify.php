...
return [
    ...
    */
    ...
    // Prefer webhook-warmed local cache/DB over live Shopify API on storefront reads.
    // Checkout/cart should still call fetchProductByHandle(..., fresh: true).
    'storefront_local_first' => filter_var(env('SHOPIFY_STOREFRONT_LOCAL_FIRST', true), FILTER_VALIDATE_BOOLEAN),
    ...
    // Single product page + catalog cache (seconds). Overridden by STOREFRONT_SHOPIFY_CACHE_TTL when set.
    'product_cache_ttl' => (int) env('SHOPIFY_PRODUCT_CACHE_TTL', 3600),
    ...
    'tax_cache_ttl'  => (int) env('SHOPIFY_TAX_CACHE_TTL', 1800),
    // Shipping quote cache (seconds) — local rate table + free-ship threshold
    'shipping_cache_ttl' => (int) env('SHOPIFY_SHIPPING_CACHE_TTL', 1800),
