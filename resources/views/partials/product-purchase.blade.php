...
<div>
  <div class="flex items-center gap-4 mb-4">
    <div class="flex items-center border-2 border-slate-200 rounded-xl overflow-hidden">
      <button
        type="button"
        @click="decrementQty()"
        class="px-4 py-3 text-slate-600 hover:text-navy-700 hover:bg-slate-50 transition-colors font-bold text-lg"
      >−</button>
      <span class="px-5 py-3 font-semibold text-navy-900 text-lg border-x-2 border-slate-200 min-w-[3rem] text-center" x-text="qty">1</span>
      <button
        type="button"
        @click="incrementQty()"
        class="px-4 py-3 text-slate-600 hover:text-navy-700 hover:bg-slate-50 transition-colors font-bold text-lg"
    ...
    <button
      ...
      class="btn-primary-lg flex-1 justify-center transition-opacity"
    >
  ...
  <button
    ...
    :class="purchaseLinkClasses()"
    :aria-disabled="!canPurchase"
    class="btn-gold-lg w-full justify-center mb-6 transition-opacity"
  >
    {{ $orderNowText }}
