    ...
    <div class="grid lg:grid-cols-2 gap-8 lg:gap-20 items-start">
      ...
      <div class="min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-6 p-4 bg-navy-50 rounded-2xl">
          <div class="min-w-0">
            <span class="font-display font-bold text-3xl sm:text-4xl text-navy-900" x-text="(selectedOption !== null && @js($variantPricesMap)[selectedOption]) ? @js($variantPricesMap)[selectedOption].price : '{{ $fmt($displayPrice) }}'">{{ $fmt($displayPrice) }}</span>
            @if($displayCompare)
            <span class="text-slate-400 line-through text-base sm:text-lg ml-2" x-text="(selectedOption !== null && @js($variantPricesMap)[selectedOption] && @js($variantPricesMap)[selectedOption].compare) ? @js($variantPricesMap)[selectedOption].compare : '{{ $fmt($displayCompare) }}'">{{ $fmt($displayCompare) }}</span>
            @endif
          </div>
      ...
      <div class="grid md:grid-cols-3 gap-5">
        ...
        <figure class="group">
          <div class="overflow-hidden rounded-2xl aspect-[4/5] bg-stone-100 mb-3">
            <img src="{{ str_starts_with($img, 'http') ? $img : (\Illuminate\Support\Str::startsWith($img, ['http://', 'https://']) ? $img : asset('images/' . $img)) }}" alt="{{ $cap }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" loading="lazy" decoding="async" width="400" height="500">
          </div>
          <figcaption>
...
<section class="section bg-gradient-to-b from-stone-50 to-white" aria-label="Final call to action">
  <div class="container-narrow text-center">
    ...
    <div class="mb-6">
      <span class="font-display font-bold text-4xl sm:text-5xl text-navy-900" x-text="(selectedOption !== null && @js($variantPricesMap)[selectedOption]) ? @js($variantPricesMap)[selectedOption].price : '{{ $fmt($displayPrice) }}'">{{ $fmt($displayPrice) }}</span>
    </div>
  ...
  <div class="container-site pt-2 pb-2">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div class="hidden sm:block min-w-0 flex-1">
        ...
        <p class="text-navy-700 font-bold text-base" x-text="(selectedOption !== null && @js($variantPricesMap)[selectedOption]) ? @js($variantPricesMap)[selectedOption].price : '{{ $fmt($displayPrice) }}'">{{ $fmt($displayPrice) }}</p>
      </div>
      
      @if($purchaseOptions['requiresOption'] ?? $requiresOption)
      <div class="flex-1 min-w-[140px] sm:flex-none">
        <select @change="selectOption($event.target.value === '' ? null : (isNaN($event.target.value) ? $event.target.value : Number($event.target.value)))" :value="selectedOption" class="w-full bg-slate-50 border border-slate-200 text-navy-900 text-sm rounded-lg focus:ring-gold-400 focus:border-gold-400 block p-2.5 font-semibold appearance-none">
