...
@php
  ...
  $asyncCart = ! empty($asyncCart);
  $cartSummaryUrl = $cartSummaryUrl ?? route('cart.summary', ['locale' => $locale]);
  ...
  $checkoutClientConfig = [
    'asyncCart'            => $asyncCart,
    'pendingDiscount'      => $pendingDiscount ?? session('pending_discount_code'),
    'cartItems'            => $asyncCart ? [] : $cartItems,
    'pricing'              => $pricing,
    'summarySubtotal'      => $asyncCart ? 0 : $summarySubtotal,
    'summaryTax'           => $asyncCart ? 0 : (float) ($pricing['tax'] ?? 0),
    'summarySubtotalUsd'   => $asyncCart ? 0 : (float) ($pricing['subtotal_usd'] ?? 0),
    ...
    'shippingCost'         => $asyncCart ? 0 : ($pricing['shipping'] ?? 0),
    'taxAmount'            => $asyncCart ? 0 : ($pricing['tax'] ?? 0),
    ...
    'freeShipRemainingTpl' => __('checkout.free_ship_remaining', ['amount' => ':amount']),
    'paymentSuccessMessage'=> __('checkout.payment_success'),
    'squareAppId'          => $squareAppId,
    'squareLocationId'     => $squareLocationId,
    'squareSdkUrl'         => ($squareEnv === 'sandbox')
...
{{--
  ...
  Square SDK is lazy-loaded only when the shopper reaches the Payment step.
--}}
@push('head_scripts')
    ...
    <div class="container-site flex items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3 flex-shrink-0">
        <a
          ...
          class="inline-flex items-center gap-1.5 text-sm font-medium text-navy-600 hover:text-navy-800 transition-colors"
        >
      ...
      <div class="space-y-4 lg:sticky lg:top-24" id="checkout-order-summary">
        <div class="card p-6">
          ...
          <div class="space-y-4 mb-5 max-h-80 overflow-y-auto overflow-x-visible pr-1 pt-1">
            {{-- Skeleton while AJAX cart loads --}}
            <div x-show="cartLoading" class="space-y-4 animate-pulse" x-cloak>
              <template x-for="i in 2" :key="'sk-'+i">
                <div class="flex items-start gap-3">
                  <div class="w-16 h-16 rounded-xl bg-slate-200 flex-shrink-0"></div>
            ...
            <template x-for="item in cartItems" :key="item.key">
              <div class="flex items-start gap-3" x-show="!cartLoading">
            ...
            <p x-show="!cartLoading && cartItems.length === 0" x-cloak class="text-slate-500 text-sm">{{ __('checkout.cart_empty') }}</p>
          </div>
        ...
        <div class="card p-5 space-y-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-slate-400 text-xs" x-text="freeShippingHint()"></p>
            </div>
          </div>
          <div class="flex items-center gap-3">
